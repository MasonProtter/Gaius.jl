...
const Eltypes  = Union{Float64, Float32, Int64, Int32}
...
@inline PtrMatrix(A::SubArray{T,2,Array{T,<:Any},<:Tuple{Int64,Vararg}}) where {T} = SparseStridePointerMatrix(pointer(A), size(A), strides(A))
...
@inline stridedpointer(A::SparseStridePointerMatrix) = SparseStridePointerMatrix(A.ptr, A.strides)
